...
func (t *blobserveTransport) redirect(image string, req *http.Request) (*http.Response, error) {
	...
	var location string
	if t.Config.Config.GitpodInstallation.WorkspaceHostSuffix != "" {
		location = fmt.Sprintf("%s://%s%s/%s%s%s",
			t.Config.Config.GitpodInstallation.Scheme,
			"blobserve",
			t.Config.Config.GitpodInstallation.WorkspaceHostSuffix,
			image,
			imagePathSeparator,
			path,
		)
		...
		location = fmt.Sprintf("%s://%s/%s/%s%s%s",
			t.Config.Config.GitpodInstallation.Scheme,
			t.Config.Config.GitpodInstallation.HostName,
			"blobserve",
			image,
			imagePathSeparator,
			path,
